...
- `PBSMS_Tenant_AI_Assistant_Ch47_v2_0.pdf` — SRS v2.1 Chapter 47, a subscription-gated per-tenant AI assistant. A **product feature** (schools pay for and use it). **Supersedes v1.0 as of 2026-08-27** — v2.0's own status header genuinely says "ADOPTED BASELINE — APPROVED FOR BUILD," no CLAUDE.md/PDF drift this time. New §47.0–§47.0.3 add amendment rules and a staged build-authorization table (see the Internal Engineering Agent section below for exactly what's cleared); the requirement text itself (§47.1 onward) is unchanged from v1.0. §47.18's open questions (no model provider selected, DP-103/DP-107 unresolved) explicitly survive adoption per §47.0.3 — approval didn't close them. `v1_0` kept for history.
- `PBSMS_Internal_Engineering_Agent_v2_0.pdf` — the process spec the "Internal Engineering Agent" section below implements. **Not a product feature** — governs how an AI coding assistant (this one) may work in this repository. **Supersedes v1.1 as of 2026-08-27** — v2.0's own status header genuinely says "ADOPTED — APPROVED FOR IMPLEMENTATION," no drift. New §1.3 governs amendment (**EC-005, EC-302 and EC-400 are explicitly not amendable by the ordinary process** — changing any of the three requires re-adoption of the whole document) and gives a staged implementation-authorization table (see below). No EC rule's substance changed from v1.1 to v2.0. (`PBSMS_Internal_Engineering_Copilot_v1_0.*` and `PBSMS_Internal_Engineering_Agent_v1_1.*` kept for history.)
...
- **EC-005, EC-302 and EC-400 are not amendable by the ordinary process at all.** Changing any of the three requires re-adoption of the whole document — not a CLAUDE.md edit, not an in-session instruction, no matter who gives it. They are the three rules whose entire value is that they cannot be relaxed by whoever is under pressure at the time.
- **Correction, 2026-08-27**: earlier the same day, before v2.0 arrived, this Agent drafted and opened a PR amending EC-005 in CLAUDE.md directly, following an explicit in-session instruction from the repo owner. That PR (#10) is invalid under this rule and was closed without merging once v2.0's actual §1.3 language was read. The mistake was treating EC-005 as EC-201-style (a rule with documented precedent for a narrow, human-authorized exception) rather than checking whether it carried its own, stricter amendment rule — it does. Lesson: for a rule this consequential, verify the amendment mechanism the *governing document itself* specifies before drafting any change, rather than reasoning from a different rule's precedent.
...
|---|---|---|
| 1 | Retrieval layer under RLS, scope enforcement, audit logging — no model | **CLEARED — begin immediately. No external dependency.** |
| 2 | Evaluation harness, golden sets, adversarial corpus, CI gates | **CLEARED — begin immediately, in parallel with stage 1.** |
| 3 | Ask capability, staff roles, internal reference tenant | GATED on a model provider contractually satisfying DP-103 (zero retention, no training, no human review) and DP-107 (disclosed processing jurisdiction) — unresolved, survives adoption per §47.0.3. |
| 4 | Shadow period, then canary tenants | GATED on stage 3 and §47.15.2 thresholds green. |
| 5–7 | Draft; demo mode and metering; Explain and Find | GATED on the preceding stage. |
| 8 | Parent View exposure | **NOT AUTHORISED by this adoption.** Separate decision, separate risk review. |
...
Stages 1–2 are cleared deliberately and should start first — they're the isolation boundary and the evaluation harness, the two things that can't be retrofitted, and neither depends on the provider decision gating everything after. **Being cleared to build does not change who may build it**: EC-005 above still absolutely bars this Agent from authoring the scope-configuration, retrieval-rules, or evaluation-gate code even at Stage 1–2 — a human author is still required for those specific artefacts. The implementation plan drafted 2026-08-27 (Stage 1–2 retrieval/scope/audit substrate, no model, one vertical slice on attendance) remains a valid handoff specification for whoever authors it; this Agent may still draft the non-artefact scaffolding around it (e.g. the `assistant_interactions`/`assistant_settings` migration, thin module wiring) under the same review posture as finance/results code, but not the retrieval service's scope logic, the DP-100 category allowlist, or the isolation test suite itself.
