...
**What's still not built, on purpose**: the actual ingestion/clustering job (EC-100/101 — reading this table on a schedule, deduplicating, writing GitHub issues) and any read/review surface for it. Stated, honest limitation from the migration's own header: EC-300's request to strip student/guardian names etc. from feedback *content* is a named-entity-recognition problem for free text, not something a migration or a regex can honestly claim to solve — the submission form asks reporters not to include a name, nothing yet enforces it mechanically. Anyone building the ingestion job needs to solve real redaction of the `subject`/`message` fields first; the opaque `tenant_ref` alone is not the whole anonymisation story.
