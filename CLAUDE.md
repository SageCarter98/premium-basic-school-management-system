...
- `PBSMS_Tenant_AI_Assistant_Ch47_v1_0.pdf` — SRS v2.1 Chapter 47, a subscription-gated per-tenant AI assistant. A **product feature** (schools pay for and use it). Reviewed and approved by the Engineering Lead on 2026-08-24 — a real decision — but **the PDF's own internal status header was never updated to match**: it still reads "For review. Not approved for build. No model provider selected; see §47.18." Don't trust that field over this line. Approving the chapter didn't resolve §47.18's own still-open, architecture-constraining questions, independent of the approval label: no AI model provider has been selected, and DP-103 (contractual zero-retention) and DP-107 (residency disclosure) remain undecided — the chapter's own text says these should be settled before implementation starts, since they shape the architecture, not just procurement. Not yet built.
- `PBSMS_Internal_Engineering_Agent_v1_1.pdf` — the process spec the "Internal Engineering Agent" section below implements. **Not a product feature** — governs how an AI coding assistant (this one) may work in this repository. Reviewed and approved by the repo owner on 2026-08-24, same as above — its own PDF status header was likewise never updated and still reads "For review. Not approved for adoption." This file, not the PDF's internal metadata, is the one that actually binds this Agent's behavior. (`PBSMS_Internal_Engineering_Copilot_v1_0.*` is its immediate predecessor, kept for history — same rules, old name, superseded by the rename in v1.1's revision note.)
...
Reviewed and approved 2026-08-24. Full text: `PBSMS_Internal_Engineering_Agent_v1_1.pdf` — its own internal status header was never updated to reflect that approval and still reads "for review"; see the Repository state section above. This section is the "Agent rulebook" component that document's own §1.2 calls for — this file, read on every invocation, is what makes the rules binding rather than aspirational.
