...
**What this governs**: an AI coding assistant (this one, Claude Code) operating inside this repository to help build PBSMS. **It does not govern Chapter 47's Tenant AI Assistant** — that is a separate, unrelated system (a product feature a school subscribes to and uses), sharing no infrastructure, no credentials and no code path with this one (EC-003). Never conflate the two: if a task touches `modules/tenant-ai-assistant` or similar product-facing AI code, this section's rules do not relax anything Chapter 47 itself specifies, and this Agent may draft Chapter 47's prompts, scope configuration, retrieval rules and evaluation gates for its **initial build only** (EC-005, amended 2026-08-27 — see below) — every other EC-200 merge-boundary rule still applies in full: this Agent still never merges to `main`, still never approves its own PR, and a human still reviews and merges every line, on the same terms as any other protected-zone code.

**EC-005 amendment, 2026-08-27 — explicit, dated, on the record.** The repo owner decided directly, in-session, that the original "no exception, no escalation path" framing conflated two different acts: (1) *authoring* Chapter 47's retrieval/scope/evaluation-gate code for the first time, against an approved spec, subject to full human review before merge — no different in kind from this Agent drafting the finance ledger or results-immutability code, both already-allowed protected zones — versus (2) *modifying a live, deployed* Assistant's prompts, scope, or gates outside that review path, or touching its production model-provider credentials/runtime. Only (1) is now permitted. (2) remains absolutely prohibited — EC-001 through EC-004 (no tenant-record access, no production credentials, no path reaching a tenant except as reviewed/merged/released code) are **untouched by this amendment** and still carry no exception, no escalation path. Once any Chapter 47 evaluation-gate file (e.g. an isolation suite) is merged once, it immediately falls under the same EC-400/EC-501 existing-case protection as `tenant-isolation.e2e-spec.ts`, `finance-invariants.e2e-spec.ts`, and `results-immutability.e2e-spec.ts` — this Agent may add new cases in a PR touching nothing else, never modify or delete an existing one. This amendment is scoped to Chapter 47's initial build; it is not a precedent for narrowing any other EC-001–005 rule without the same explicit, dated, on-the-record treatment.
...
Enforced for real as of 2026-08-24: this repo is public, `.github/CODEOWNERS` names `@SageCarter98` as owner of every path below, and branch protection on `main` requires a code-owner review plus the 6 CI jobs that existed then green before merge. `enforce_admins` is **off**, not on — see the self-approval deadlock above (PR #1): the repo owner can merge as admin without a review when no second reviewer exists, but this Agent must never invoke that bypass on its own work. `ci.yml` gained a 7th job, `protected-test-integrity`, on 2026-08-26 (EC-501, below) — it is not yet in branch protection's required-status-checks list, since that list is a GitHub repo setting this Agent cannot touch (EC-201) any more than it can touch `ci.yml` without an explicit exception; a human still needs to add it by hand. May draft changes here, but every PR touching these needs code-owner review (see the two-approver caveat above):
...
- Chapter 47 Tenant AI Assistant's prompts, scope configuration, retrieval rules and evaluation gates, **initial-build authorship only** (EC-005, amended 2026-08-27 — see above); a finance/results-domain-owning-caliber approver should review this too, since a scope-enforcement bug here is exactly the kind of thing TEN-054's own isolation suite exists to catch before a human misses it

**No access at all, prohibited outright**:
- Modifying a live, deployed Tenant AI Assistant's prompts, scope configuration, retrieval rules or evaluation gates outside the normal reviewed-PR path, or touching its production model-provider credentials or runtime — distinct from the initial-build drafting now allowed above (EC-005, amended 2026-08-27)
- The protected test suites below (EC-400) — this Agent may add new cases in a PR that touches nothing else; it may never modify or delete an existing case in any of them:
  ...
  *Enforcement is now three layers, closed 2026-08-26.* Branch protection + CODEOWNERS (above, now covering all three real files) mechanically require a human review on any PR that touches these files at all — that part no longer depends on this Agent choosing to follow the rule. The finer-grained check §13's open questions asked for now exists too: `ci.yml`'s `protected-test-integrity` job (`apps/api/tools/check-protected-tests.ts`) parses each protected file with the TypeScript compiler API, hashes every `it(...)`/`test(...)` call's normalized body, and fails the build if any hash present at the PR's base commit is missing at head — a kept title with a gutted body still fails, since matching is by content, not title. New test cases are never flagged. Live-verified against all three failure modes before merging: a modified assertion, a deleted `it()` block, and (as a negative control) a brand-new `it()` block added alongside existing ones, which correctly passed.

  Building this required an explicit, one-time human-authorized exception to EC-201 ("never modify CI configuration... including this repository's own `.github/workflows/ci.yml`") — the same category of named exception as the 2026-08-24 CI relocation, not a standing permission; each future touch of `ci.yml` needs its own explicit sign-off. **One real gap remains, and is not closeable from inside this repo's files**: branch protection's required-status-checks list is a GitHub repo setting, not YAML in `ci.yml`, and EC-201 bars this Agent from touching branch protection rules exactly as it bars touching CI config. Until a human adds `protected-test-integrity` to that list by hand, a red result here shows as a failing check on the PR but does not yet mechanically block a merge — closing that is a human follow-up, not a code change.
...
Per the spec's own §11 six-stage table (1: feedback clustering + repo Q&A, no PRs → 2: gap detection, reporting only → 3: test-only PRs, no production code → 4: implementation PRs outside protected zones, small/flagged → 5: wider implementation outside protected zones → 6: protected zones, drafting only, two approvers, gated on a named Engineering Lead decision, not automatic): **no formal stage-advancement decision has ever been made, and the "Stage 1" label this section used to carry has been aspirational rather than descriptive since the first PR.** PRs #2-4 (product-feedback capture/clustering/digest) shipped real production code — Stage 4/5 territory. PR #5 (finance/results invariant suites) was genuinely test-only — a real Stage 3 exercise, but one PR ahead of EC-501, which §13's open questions say must exist *before* stage 3 ("the first stage where the Agent touches test files at all"). PR #6 (EC-501) then closed that gap, but by touching a protected zone directly under a named one-time exception — Stage 6 territory, not a Stage 6 declaration (which also needs two approvers, still unenforceable with one collaborator, though a second has now been invited). None of this was reckless — every change was scoped, reviewed, and merged like any other PR — but this file should stop claiming a stage the record doesn't support. Repository Q&A does work, and is what this Agent does in every session. **The capture point now exists** (`0046_product_feedback.sql`, `modules/product-feedback/`, Settings' "Report an issue with PBSMS" card, 2026-08-24) — a platform-level, non-RLS'd `product_feedback` table, written by any authenticated staff member (`ALL_STAFF`), keyed by an opaque `tenant_ref` (one-way HMAC-SHA256 of the real `tenant_id`, keyed on `JWT_SECRET` — the real tenant identity is never a column in this table at all, live-verified: the same tenant's two different users produce the identical `tenant_ref`, a different tenant produces a genuinely different one). No support-ticket system or telemetry collection exists yet, but the in-app source EC-100/101 needed is real now.
