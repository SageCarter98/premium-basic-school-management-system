...
**What's still not built, on purpose**: the actual scheduled ingestion/clustering *job* (EC-100/101 literally asks for a job, not just queries a human runs by hand) and any GitHub-issue-writing automation — the queries above are a human (the Engineering Lead) running them manually today, not automation yet. Stated, honest limitation carried over from the capture point itself: EC-300's request to strip student/guardian names etc. from feedback *content* is a named-entity-recognition problem for free text, not something a migration, a regex, or a SQL clustering query can honestly claim to solve — the submission form asks reporters not to include a name, nothing yet enforces it mechanically. Anyone automating the ingestion job needs to solve real redaction of the `subject`/`message` fields first; the opaque `tenant_ref` alone was never the whole anonymisation story.
