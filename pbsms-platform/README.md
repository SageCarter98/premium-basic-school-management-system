...
| Tenant-context middleware, request-scoped tenant-safe DB access with a leak-free connection lifecycle (see bug #9) | Full Chapter 13.3 record-relationship scope (assigned students/linked children/record ownership) — guardians and teacher-assignments are now real entities (`modules/guardians/`, `modules/teacher-assignments/`) but the broader scoping/enforcement itself (e.g. a teacher's endpoint access restricted to only their own assigned classes) isn't wired to anything yet |
...
| Login issuing a correctly tenant-scoped JWT, now carrying `role_codes` (Chapter 13/33 Pass 1), rate-limited (5 failed attempts/15 min → 429, scoped per-account, blocks even a correct password once tripped) and MFA-capable (hand-rolled RFC 6238 TOTP — no new dependency — enroll/enable/verify endpoints, a short-lived MFA-challenge token that carries no tenant claims so it can never be used as a real access token). **MFA is now policy-enforced (SEC-030), not just mechanism-complete — closed 2026-08-12**: `LEADERSHIP` roles (proprietor/administrator/headmaster, reused from `role-groups.ts` rather than a second hardcoded list) that haven't enrolled MFA yet no longer get a full accessToken on login — they get one scoped to exactly `/v1/auth/mfa/enroll`+`/v1/auth/mfa/enable` (`tenant.middleware.ts`'s new `MFA_SETUP_PATHS` check, gated on a new `mfaSetupRequired` JWT claim), refused on every other endpoint until they complete setup; `enableMfa()` re-issues a full token on success so completing enrollment doesn't also require a second login. This is a real bootstrap path, not a bare lockout — verified live end to end (login → 401 on `/v1/staff` with the setup-only token → enroll → a real TOTP code computed by hand-porting `totp.ts`'s algorithm to a throwaway script → enable → 200 on `/v1/staff` with the re-issued token → a second login now correctly returns the MFA-challenge flow instead of setup-required; a non-mandatory role like `teacher` is unaffected, plain accessToken immediately) | Refresh tokens/session revocation, password reset; platform-role MFA enforcement isn't reachable yet either way since `tenant.middleware.ts` already rejects all platform-role requests until TEN-020/021 impersonation exists |
...
**Next**: the original 7-phase plan's remaining items — B (rest of
Chapter 33 auth: refresh tokens/session revocation/password reset), C
(Chapter 13.3/13.4 record-relationship scope + delegation — unblocked
since guardians and teacher-assignments both exist), D (Chapter 35
background jobs/scheduler — would also finally unlock A4's deferred
notification piece), E (Chapter 14 KPI engine), F (Chapter 39-40 data
protection), G (Chapter 41 NaCCA). Report back and confirm before
