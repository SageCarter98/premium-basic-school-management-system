...
| **Product feedback capture point (`0046_product_feedback.sql`, `modules/product-feedback/`) — closed 2026-08-24**: the first real prerequisite for the Internal Engineering Agent process spec's EC-100/101 (`PBSMS_Internal_Engineering_Agent_v1_1.pdf`, `CLAUDE.md`'s own "Internal Engineering Agent" section) — a channel for feedback about PBSMS *the product itself*, cross-tenant, deliberately distinct from `staff_feedback` (0044), which stays internal-to-one-school on purpose and was explicitly NOT reused for this (a real design question the user raised and a real design mistake to avoid: repurposing an internal-complaints channel as cross-tenant engineering signal nobody consented to that use for). Platform-category table, no `tenant_id` column, not RLS'd — `tenant_ref` is a one-way HMAC-SHA256 of the real tenant id keyed on the existing `JWT_SECRET` (no new secret minted), computed application-side before insert; `pbsms_app` gets an INSERT-only grant, nothing reads it back through the app yet. `POST /v1/product-feedback` (`ALL_STAFF`, any authenticated staff member) takes category/subject/message/screen. **Honest, stated limitation, not silently glossed over**: EC-300 also asks that student/guardian names be stripped from feedback *content* — real named-entity-recognition on free text, which a migration or a regex can't honestly claim to solve; the submission form asks reporters not to include a name, nothing yet enforces it mechanically. Live-verified: the same tenant's two different users (different roles) produced byte-identical `tenant_ref` values while a second tenant's was genuinely different, confirmed directly in Postgres — the "know that fourteen tenants reported this, not which fourteen" property EC-101 asks for, proven rather than assumed. **Clustering approach settled the same day, `0047_product_feedback_clustering.sql`**: database-native (`pg_trgm` similarity + Postgres's built-in `ts_stat` full-text stats), not embeddings or an external NLP call — chosen specifically so no feedback text ever leaves this database, sidestepping the same kind of provider-vetting decision Chapter 47's DP-103 required for the product-facing Assistant. `infra/queries/product_feedback_clustering.sql` is a runbook of 5 ready-to-run queries (volume/reach per category, affected roles via `unnest(role_codes)`, top vocabulary per category, near-duplicate report pairs, and grouped near-duplicates), each live-verified against realistic inserted test data before being committed — not just syntax-checked. `pbsms_platform` gets a least-privilege SELECT grant matching the same category as every other platform-review grant in this codebase. **Real, stated ceiling**: neither technique is true semantic understanding — two reports describing the same bug in genuinely different words will not be linked | The actual EC-100/101 ingestion/clustering *job* (closed below) and any GitHub-issue-writing automation — deliberately still not built; real redaction of free-text `subject`/`message` content, needed before automation goes further than this |
| **EC-100/101 scheduled product-feedback digest job (`0048_product_feedback_digest.sql`, `jobs-worker/handlers/product-feedback-digest.handler.ts`) — closed 2026-08-26**: the piece the row above explicitly deferred — turns the runbook's five queries from something a human runs by hand into a real scheduled job. Deliberately does **not** reuse `background_jobs`/`job_schedules` (0027, Phase D) — that queue is fundamentally per-tenant (RLS'd, `dequeue_next_job()` always scoped to one tenant), while `product_feedback` has no `tenant_id` at all; forcing a cross-tenant computation through a per-tenant queue would model it wrong. Instead `src/worker.ts` gained a fourth, independent timer (`WORKER_FEEDBACK_DIGEST_POLL_MS`, default once/day) that calls the new handler directly against a plain `pbsms_worker`-role pool — no `TenantContextStore`/`WorkerTenantConnection` involved, since there's no tenant to scope to. Each run is a full snapshot (all reports to date, not a delta) written as one `product_feedback_digests` row — same "recompute the whole current picture" shape Chapter 14's `kpi_snapshots` already established — and the handler skips writing anything when there's been no new feedback since the last run, so a quiet day doesn't spam identical rows. `pbsms_worker` gets plain `SELECT` on `product_feedback` and `SELECT`+`INSERT` on the new table (no `SECURITY DEFINER` trick needed — neither table is RLS'd, so there's no per-tenant bypass problem here unlike 0027's functions); `pbsms_platform` gets read access to the digest table, same shape as its existing grant on `product_feedback` itself. Live-verified: inserted synthetic (non-PII) feedback rows spanning near-duplicate phrasing and multiple categories/roles, ran the handler directly against local Postgres, confirmed a real digest row with correct category counts, role breakdowns, `ts_stat` vocabulary, and a genuine near-duplicate cluster pairing the two similarly-worded reports; confirmed a second run with no new rows correctly skips (no duplicate row written); confirmed `pbsms_app` still cannot read either table (grant-shape unchanged for that role). Schema reset to clear the synthetic rows afterward, same discipline as every other live-verified module. **Code review (`/code-review`) caught 5 real issues before merge, all fixed same day**: (1) `count(*)::text` cast before `ORDER BY` sorted rankings lexically, not numerically — `"12"` ranked below `"3"`/`"5"`, silently inverting "top category/role/duplicate" the moment any count hit double digits; fixed by ordering on the untouched `count(*)` instead of the text alias, live-reverified with an 11-vs-3 split to prove `bug` now correctly outranks `feature_request`. (2) The digest's separate read queries (total count, last-digest check, new-count, then the four summary queries) weren't transactionally consistent against a live, concurrently-written table; fixed by wrapping the entire run in one `REPEATABLE READ` transaction on a single checked-out connection. (3) The per-category vocabulary loop iterated a hardcoded `['bug','feature_request','other']` list instead of the table's own distinct values, which would have silently dropped any future new category; fixed by deriving categories live from `product_feedback` each run (also required moving the category filter into a `format()`-built, properly `%L`-quoted dynamic query rather than trusting a fixed whitelist). (4) A dead `having count(*) > 0` clause in the duplicate-cluster query, removed. (5) The summary queries were flagged as unnecessarily sequential — but fixing (2) onto a single transactional connection makes that moot: one Postgres connection only ever runs one query at a time, so real concurrency would need separate connections, which would break (2)'s consistency guarantee; documented in the handler rather than "fixed" by adding illusory `Promise.all` parallelism. Re-verified after fixes: clean build/lint, full fresh reset+migrate+seed+495/495 e2e suite | Still not built, on purpose, same as the row above: any GitHub-issue-writing automation, and real redaction of free-text `subject`/`message` content before this or any future automation reads it more actively than a snapshot digest does |
| **Finance invariant and results-immutability test suites (`apps/api/test/finance-invariants.e2e-spec.ts`, `apps/api/test/results-immutability.e2e-spec.ts`) — closed 2026-08-26**: built while scoping EC-501 (the CI job that will detect a deleted/weakened test case in a protected suite) — CLAUDE.md's EC-400 had named both suites as already-protected for two days before either file actually existed, a real documentation-drift instance, not a hypothetical. Neither is a tenant-isolation test (that's `tenant-isolation.e2e-spec.ts`'s job); these assert business/money and record-immutability invariants that only live in `finance.service.ts`/`results.service.ts`'s own logic. Harness: both services take a `Scope.REQUEST` `TenantDatabaseService`, so neither can be `new`'d against a plain `Pool` the way `tenant-lifecycle.e2e-spec.ts` does for `TenantsService` — both suites instead reuse `WorkerTenantConnection` (built for worker.ts's job handlers, the identical "need a real TenantDatabaseService outside an HTTP request" problem) wrapped in `TenantContextStore.run()` per call. Finance: 12 tests covering maker-checker distinctness, balance-at-approval-time capping, reversal non-mutation + one-time-only (service and DB level), invoice cancellation scope, both allocation caps plus the assistance-blind-spot gap below, settlement one-payment-one-line, fee-structure activation sum equality, and penalty truncation. Results: 7 tests covering the full legal status-transition path plus illegal-transition rejections, `returnForCorrection()`'s reopened-from-approved path, `reopen()`'s supersede-not-mutate mechanics, the partial unique index and grading-scale `EXCLUDE` constraint as direct DB-level proofs, and one deliberate **stated-ceiling test** proving a raw SQL `UPDATE` against a published row succeeds today — `pbsms_app` has an unrestricted grant on `student_results`, no trigger or status-transition `CHECK` exists, so "immutability" beyond the two DB-backed constraints is application-code discipline, not a structural guarantee, and the suite says so rather than implying otherwise. **Three real, previously-unknown gaps surfaced while writing/reviewing these tests, documented in the suites themselves rather than fixed** (all are protected-zone code; a test suite isn't the place to fix a live invariant bug): (1) `allocate()`'s invoice-side balance cap doesn't exclude allocations tied to a reversed payment the way `findInvoiceBalance()` does, so post-reversal the two balance computations disagree — `allocate()` incorrectly rejects an amount `findInvoiceBalance()` reports as free; (2) a second, more severe divergence found during `/code-review` of this same PR and confirmed live before being written up: `allocate()`'s invoice-side cap also never subtracts approved `financial_assistance` at all, unlike `findInvoiceBalance()`/`finalizeAssistanceApproval()` — needs no reversal to trigger, just ordinary combined assistance-plus-payment use; confirmed live with a 1000 invoice + 600 approved assistance (true remaining 400) still letting a 500 allocation through, overcommitting the invoice by 100; (3) `reopen()`'s precondition only checks `status in ('published','locked')`, never `superseded_at`, so reopening an already-reopened (stale) row passes that check and only fails via a raw, unhandled unique-constraint violation instead of a clean `ConflictException`. **Real cross-suite pollution caught and fixed before this was done, in two layers**: (1) a first version with no cleanup left every fixture (fee structures, invoices, payments, student_results, ...) sitting in the database, breaking `tenant-isolation.e2e-spec.ts`'s own exact-row-count assertions for every finance table plus `student_results`/`student_result_items` — fixed with `afterAll` cleanup tracking every created id, mirroring `tenant-lifecycle.e2e-spec.ts`'s own `createdTenantIds` precedent. (2) Even with that cleanup in place, a genuinely fresh reset+migrate+seed run still failed the same way intermittently — Jest runs every `*.e2e-spec.ts` FILE in a separate worker process by default, so `tenant-isolation.e2e-spec.ts` could execute concurrently with `finance-invariants.e2e-spec.ts`/`results-immutability.e2e-spec.ts` and read the database mid-test, before either file's `afterAll` had run; no amount of end-of-file cleanup fixes a race against tests still in flight. Fixed by adding `--runInBand` to `apps/api/package.json`'s `test:e2e` script, forcing all e2e files to run serially against the shared database — this was never a problem before because `tenant-lifecycle.e2e-spec.ts` (the only other non-isolation e2e file) touches entirely disjoint tables (`tenants`/`platform_audit_logs`), so nothing before this pair of suites ever shared write-visible state across files. Verified by running the full 4-file suite three consecutive times with no reset in between, both before this fix (still flaked) and after (clean every time, ~2x faster too — 22-31s vs 45-90s — since tests stop contending for the same Postgres connections). `jest-e2e.json`'s `testTimeout` was briefly bumped to 30000ms while `--runInBand` was still being isolated as the real cause, then reverted back to the unconfigured 5000ms default once 3 consecutive clean runs confirmed `--runInBand` alone was sufficient — deliberately not left widened as a side effect, since that would have quietly weakened fail-fast behavior on `tenant-isolation.e2e-spec.ts` for an unrelated reason | EC-501 itself (the CI diff-check that will make these three files' protection mechanical rather than review-dependent) — a separate, not-yet-started PR; fixing any of the three documented gaps above |
...
| **Chapter 47 (Tenant AI Assistant) Stage 1 governance-runtime substrate — closed 2026-08-27, fixed 2026-08-28**: `modules/tenant-ai-assistant/` — `AssistantRetrievalService.findLowAttendance()` (§47.0.2's own Stage 1 vertical slice, the attendance-below-threshold worked example from §47.3.1), `AssistantSettingsService` (the tenant/role disable NFR, checked fresh on every call — no cache, so "takes effect immediately" needs no invalidation machinery), `AssistantInteractionLogger` (FR-AIT-600's audit row, with `question_text`/`response_text`/`model_version`/`validation_outcome`/`latency_ms`/`cost_*` all nullable and left null at this stage — no model in the loop yet). DP-100 enforced as a structural allowlist (`assistant-categories.ts`: a category not listed is unreachable, not a per-query check to remember); DP-102 as column-level minimality in the SQL itself (never `select *`); TEN-051 scope reuses `TeacherAssignmentsService.getCallerScope()` rather than reimplementing it; TEN-055 disables the Assistant outright during impersonation. Authored under EC-005's 2026-08-27 amendment (Chapter 47 artefacts may be Agent-drafted under two-approver-plus-domain-owner review) — `STAGE-1-SPEC.md` in the same directory is the design record, not a stale TODO. New 16-case `tenant-ai-assistant-isolation.e2e-spec.ts` became an EC-400 protected suite the moment it merged (CODEOWNERS and `check-protected-tests.ts` both updated in the same PR). Two real follow-up fixes the next day: a Jest-hang in the suite's own `afterAll` cleanup, and 5 real bugs the suite surfaced once it actually ran end-to-end against Postgres. Checked 2026-08-29 against the later-adopted Ch47 v2.1: none of v2.1's new requirements (Directly Supplied Content, FR-AIT-903, the block-rate signal) touch a zero-LLM, zero-free-text, single-endpoint slice like this one — still accurate against the current baseline. Under v2.1's renumbered build-authorization table this content is Stage 4-5, not "Stage 1" (v2.1's own Stage 1 is model definition/constitution, unrelated) — a stale label only in the spec file's own header comment, not in anything actually enforced | Everything else in Chapter 47: the Ask capability's real model integration (gated on an unresolved provider decision, DP-103/DP-107), Draft/Explain/Find/Research/General Conversation/Combined Analysis, Stage 0.5 demand validation (new in v2.1, not yet started — needs a provider/credentials decision before it can), Parent View (not authorised by any adopted version) |
